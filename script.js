...
function indemnizacionGeneral(){
    ...
    if (mesInicial <= mesFinal) {
        ...
        rubroB.innerText = "Según su salario de $"+ salario + ", y por sus "+ (yearFinal - yearInicial) +" años de relación, le corresponde una indemnización aproximada de $" + ((yearFinal - yearInicial)-1) * salario + " de acuerdo al art. 245 LCT";
    ...
    if (diaFinal>=1 && diaFinal<31 && mesFinal == 1 || mesFinal == 3 || mesFinal == 5 || mesFinal == 7 || mesFinal == 8 || mesFinal == 10 || mesFinal == 12) {
        ...
        rubroC.innerText = "Por intergración del mes de despido, debido a que le faltaban " + (claseMeses[0]- diaFinal) + " días y su sueldo proporcional era de $"+ (salario/claseMeses[0]) +" para que concluya el mes, le corresponde aproximadamente de acuerdo al art. 233 LCT $" + (salario/claseMeses[0])*(claseMeses[0]-diaFinal);
